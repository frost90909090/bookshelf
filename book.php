...
<html>
	...
	<body>
					...
					if(!$conn){
						...
						if(mysqli_num_rows($result) == 1){
							while($row = $result -> fetch_assoc()){
								$btn = $row['pasta'];
								$html = "<div>
										<input type=\"image\" id=\"capa\" src=\"albums/" . $row['pasta'] . "/" . $row['capa'] . "\" onclick=\"window.location='http://192.168.0.3/albums/" . $row['pasta'] ."/index.html'\"/> 
									</div>
									<div id=\"info\">
		...
		<?php
			echo "<button class=\"btn btn-warning\" id=\"readBtn\" onclick=\"window.location='http://192.168.0.3/albums/" . $btn . "/index.html'\">Ler</button>";
		?>
	</body>
</html>
